...
- **v6: 全系统统一使用 `illuminance`**。硬件只发 `illuminance`，模拟器 `SENSOR_TYPE_FIELDS` 也移除 `lightIntensity`。传感器数据表和趋势查询统一用 `$.illuminance`，数据库有 `illuminance_val` 生成列 + 索引加速 JSON 查询
- 趋势查询 `DashboardService.sensorTypeForMetric()` 按指标名动态映射传感器类型
...
| Service | 职责 |
|---|---|
| `SensorDataServiceImpl.saveAndAutoControl()` | **核心方法**：保存数据（用 `EntityManager.persist()` 直写，绕过 `SimpleJpaRepository.save()` 避免嵌套 `@Transactional` 标记 rollback-only） → 更新心跳 + 设备恢复在线自动解除告警 → 告警检测（按 sensorType 分流） → `SELECT ... FOR UPDATE` 悲观锁读设备 → **unknown 状态恢复** → 多传感器策略（single/average） → 迟滞双阈值联动决策 → **MQTT cmd 下发 + WS 推送延后到 `TransactionSynchronization.afterCommit()`**（不再阻塞 DB 连接） |
| `ControlServiceImpl` | 设备控制（开关灯/模式切换/阈值/传感器策略），通过 light 传感器 cmd 主题下发 |
| `DashboardService` | 仪表盘数据聚合。`getStats()` 用 1 条合并 SQL（`SUM(CASE WHEN...)`）返回 device 全部统计，避免多次 COUNT 往返远程 DB。`getLatestSensorData()` 按 deviceId 合并多种传感器数据；`getSensorTrend()` 通过 `sensorTypeForMetric()` 动态映射 sensorType |
...
| `HeartbeatChecker.checkHeartbeats()` | `@Scheduled(initialDelay=30000, fixedRate=5000)` 启动后延迟 30s 首次执行，之后每 5 秒检查心跳超时，标记离线并设 `lightStatus = "unknown"`，生成 OFFLINE 告警 |
...
- **3D 模式**：默认 `viewMode: '3D'`, `pitch: 55`，左上角 2D/3D 切换按钮
- **框选**：按住 Ctrl 进入框选模式（十字光标，禁用地图旋转/倾斜），拖拽框选设备，松开 Ctrl 恢复拖拽并保留选中
- **罗盘**：右下角高德原生 `ControlBar`（归北+2D），`showZoomBar: false`
- **批量操作**：选中设备后底部工具栏出现批量开灯/关灯/切换模式/设置阈值/删除/解绑
- 支持 `focusDeviceId` prop 和 `select-device` emit。暗色模式 `filter: invert(0.88) hue-rotate(180deg)`
...
## SensorData JSON 列设计 (v6)

`sensor_data` 表的 `data_json` 列存储 JSON 数据。**v6: 全系统统一 `illuminance`，去掉 `lightIntensity`**：
...
数据库增加 `illuminance_val` 生成列（`GENERATED ALWAYS AS (CAST(JSON_UNQUOTE(JSON_EXTRACT(data_json, '$.illuminance')) AS DOUBLE)) STORED`）+ 索引，趋势查询不再全表扫描 JSON。

新增索引：`idx_sd_sensor_time (sensor_id, reported_at DESC)`，`idx_sd_device_time (device_id, reported_at DESC)`，`idx_sd_reported_at (reported_at)`，`idx_sd_illuminance (illuminance_val)`，`idx_device_control_mode (control_mode)`，`idx_alarm_status_type (status, alarm_type)`。
...
mqtt.keep-alive-interval: 30            # MQTT keep-alive（秒），与心跳超时协调
deepseek.enabled: true                  # 设为 false 可禁用 AI 问答（ChatController 返回 503）
hikari.maximum-pool-size: 30            # DB 连接池上限
spring.jpa.show-sql: false              # 生产环境务必关闭 SQL 日志
```
...
以下文件/类已确认无引用并删除：`ControlModeConverter.java`、`ControlMode.java`、`DeviceStatus.java`、`LightStatus.java`、`MaxKBClient.java`、`MaxKBProperties.java`、`KnowledgeArticle.java`、`KnowledgeArticleRepository.java`。系统中设备状态/灯光状态/控制模式全程使用 String，不使用枚举。

### 无测试覆盖

当前项目没有单元测试或集成测试。`pom.xml` 引入了 `spring-boot-starter-test` 但 `src/test/` 目录为空。
